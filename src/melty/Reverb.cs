...
namespace MeltySynth {
  ...
  internal sealed class Reverb {
    ...
    private int ScaleTuning(int sampleRate, int tuning) => (int)Math.Round((double)sampleRate / 44100 * tuning);
    ...
    public float InputGain { get; private set; }

    public float RoomSize {
      get => (roomSize - offsetRoom) / scaleRoom;
    ...
    internal sealed class AllPassFilter {
      ...
      public void Mute() => Array.Clear(buffer, 0, buffer.Length);
